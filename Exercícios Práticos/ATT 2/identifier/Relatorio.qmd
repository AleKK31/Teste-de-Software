...
Este documento serve para descrever como foi o processo ao usar teste baseado em especificação para diferentes casos de teste para o método ``validateIdentifier()``.

## Passo 2 - Explorar o funcionamento do programa

Minha exploração do programa:

- Testar um caso de sucesso simples. Passando a string "A1b2";
  - <(``s``="A1b2"), (true)>
- Em seguida, testar com os limites de caracteres, 1 e 6. Vou passar a string "W" e outra "AB345f";
  - <(``s``="W"), (true)>
  - <(``s``="AB345f"), (true)>
- Testar um caso de falha. Passando um string que inicie com um número, vou usar a string "1b3";
  - <(``s``="1b3"), (true)>
...
``s`` pode ser qualquer string, mas deve comecar com uma letra e conter apenas letras ou dígitos. Além disso, deve ter no minimo 1 caractere e no maximo 6 caracteres de comprimento. Observar casos de teste excepcionais, como nulo e vazio, também vou testar o que acontece se a string tiver caracteres especiais.

- ``str`` com valor ``null``;
- ``str`` vazia;
- ``len(str)==1``; (começando com letra e com número)
- ``len(str)==6``; (começando com letra e com número)
- ``len(str)>1``; (começando com letra e com número)
- ``len(str)>6``; (começando com letra e com número)
- ``str`` conter caracteres especiais;
